...
func init() {
	packageMockFolder = "../" + constant.FOLDER_MOCK + "validate/"
}
...
func TestCheckToValidateSuccessOutput(t *testing.T) {
	...
	if output.Code != constant.OUTPUT_SUCCESS || strings.Contains(output.Debug, "Elapsed time:") == false {
		t.Errorf("Expected output %s and recëived output %s", constant.OUTPUT_SUCCESS, output.Code)
	}
	if err != nil {
...
func TestCheckToValidateSuccessOutputWithoutElapsedTime(t *testing.T) {
	...
	if output.Code != constant.OUTPUT_SUCCESS || strings.Contains(output.Debug, "Elapsed time:") {
		t.Errorf("Expected output %s and recëived output %s", constant.OUTPUT_SUCCESS, output.Code)
	}
	if err != nil {
		t.Errorf("Encountered an error while checking the output. %s", err.Error())
...
func TestCheckToValidateErrorOutput(t *testing.T) {
	...
	if output.Code != constant.OUTPUT_ERROR {
		t.Errorf("Expected output %s and recëived output %s", constant.OUTPUT_ERROR, output.Code)
	}
	if err == nil {
...
func TestCheckToValidateWarningOutput(t *testing.T) {
	...
	if output.Code != constant.OUTPUT_WARNING {
		t.Errorf("Expected output %s and recëived output %s", constant.OUTPUT_WARNING, output.Code)
	}
	if err != nil {
		t.Errorf("Encountered an error while checking the output. %s", err.Error())
	}
